...
var DataDictionary = map[string]map[string]BilingualFieldDef{
	...
	"PositionMetrics": {
		...
		"Drawdown": {
			...
			DescZH:    "负值表示正在回撤。例如：峰值较高，当前有所回落",
			DescEN:    "Negative = pulling back. E.g., Peak was higher, current has pulled back",
		},
		"Leverage": {
			NameZH: "杠杆倍数",
			NameEN: "Leverage",
...
var TradingRules = struct {
	...
	RiskManagement: map[string]BilingualRuleDef{
		"MaxPositionLoss": {
			Value:    -0.40,
			DescZH:   "单个持仓亏损达到-40%时必须止损",
			DescEN:   "Must stop-loss when single position loss reaches -40%",
			ReasonZH: "避免单笔交易造成过大损失",
			ReasonEN: "Prevent excessive loss from single trade",
		},
		"MaxDailyLoss": {
			Value:    -0.30,
			DescZH:   "单日亏损达到-30%时停止交易",
			DescEN:   "Stop trading when daily loss reaches -30%",
	...
	ExitSignals: map[string]BilingualRuleDef{
		"TrailingStop": {
			...
			ReasonZH: "锁定大部分利润，避免盈利回吐",
			ReasonEN: "Lock in most profits, avoid profit giveback",
		},
		"StopLoss": {
			Value:    -0.40,
			DescZH:   "硬止损设置在-40%",
			DescEN:   "Hard stop-loss at -40%",
			ReasonZH: "严格控制单笔最大损失",
